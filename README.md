...
| Tool | Version |
| --- | --- |
| PHP | `^8.3` (see CI matrix per Laravel version) |
| Laravel | `^11`, `^12`, or `^13` |
...
Current targets on `main` (local, PHP 8.3+):
...
CI runs the Pest **Package** suite on Ubuntu for each supported Laravel version (11–13) against the PHP versions that Laravel supports (see `exclude` in [run-tests.yml](.github/workflows/run-tests.yml): e.g. no PHP 8.5 on Laravel 11), with `prefer-lowest` and `prefer-stable`. **run-mjml-e2e-tests** uses the same PHP×Laravel matrix (8 jobs, `prefer-stable` only) with Node 24 and `npm ci`. Mutation tests run on push/PR when `src/` or `tests/` change.
